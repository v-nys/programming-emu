...
Now, you'll agree that Eratosthenes, Pythagoras and Athena are all friends. Again, so will Prolog. And you (and Prolog) will agree that Ares and Athena are friends who should be feared. Or that every one of these individuals has a friend who should be feared. Here's how you would encode the rest of the knowledge base (the “◊glossaryterm{theory}” in Prolog speak):

◊includecode["code/theory_part_two.pl" #:lang "prolog"]

Pretty straightforward, right? We'll need some basic terminology before we can start doing computations with this. First, ◊predicate{friends(X,Y)}, ◊predicate{likes(eratosthenes,primes)},... are all ◊glossaryterm{predicate}s. They are ◊glossaryterm[#:canonical "term"]{terms} which indicate a relation between their arguments: ◊glossaryterm{logic variable}s (which begin with an upper case letter), ◊glossaryterm[#:canonical "atom"]{atoms} (which begin with a lower case letter), or ◊glossaryterm[#:canonical "compound"]{compounds} (which begin with a lower case letter and have other terms as their arguments, so they look like predicates, but predicates only occur in the outermost syntactic position as far as we're concerned). That information can take the form of a ◊glossaryterm{fact}, which is just "raw" information or the form of a ◊glossaryterm{rule}, which is knowledge that can be used in a deduction. Facts are written as predicates immediately followed by a period (e.g. ◊code{likes(ares,battle).}) and rules are written as one predicate, followed by the ◊glossaryterm{implication} symbol ◊code{:-}, followed by a non-zero number of predicates again.
