...
Your typical Prolog program is also different from most programs in that, when you start it up... nothing happens. Well, one thing happens. You get a prompt. From that prompt, you can write ◊glossaryterm[#:canonical "query"]{queries}. Essentially, you can ask Prolog questions. You could ask for someone who likes mathematics by writing a predicate with a variable in it: ◊code{likes(X,math)}. And you could ask for three individuals who like mathematics by separating predicates with a comma: ◊code{likes(X,math), likes(Y,math), likes(Z,math)}.◊aside{If most of this sounds suspiciously like SQL, know that SQL is almost a logic programming language in disguise --- though most databases do not allow deduction of implicit knowledge.}
