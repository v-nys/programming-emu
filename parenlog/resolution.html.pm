...
This figure is a representation of the computations that Prolog (or Parenlog) performs. It starts at the root of the tree and expands the tree in a depth-first manner. It does so by always picking the first rule which applies to the first predicate in the query. When deepening the tree is no longer possible, the language ◊em{backtracks}. That is, it picks the second rule which applies to the first predicate in the most recent query. Again, it will expand the search tree as far as it can (and may use a third applicable rule later on, depending on the program).
...
We can write code to rename all variables in a rule (represented as an S-expression list) right now. I suggest you give it a try. If you're lost, here's how I did it:
...
But finding ◊em{that} answer may require us to apply more rules. So we have a pair of mutually recursive computations on our hands. The following diagram illustrates this:
