...
◊cmpnote/1[#:line 2]{Testing, testing.}
...
The codebase is small (really, all the actual work happens in one file) and it can be broken up into two parts: a part related to embedded language and a part related to the standalone language. It should make for a manageable project, even if you are fairly new to language development in Racket. We'll start out with a small subset of features and refactor code whenever we can't achieve parity with Jay's implementation for the maximum learning benefit.
