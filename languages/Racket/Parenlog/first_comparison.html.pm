...
◊li{Line 11 of Jay's code: This does the same, but for S-expressions in the body rather than in the head.}
◊li{Line 15 is important. Variables following the Prolog naming conventions are not quoted by ◊code{rewrite-se}, but are instead bound to unique symbols generated at runtime. If you don't see how this line works, we'll discuss it further down.}
◊li{Line 25: The extra argument ◊code{yield} to ◊code{reyield} is here for historical reasons. You do not need it.}
}
...
◊includecode["code/my-extract-stx-vars.rkt"]
◊includecode["code/extract-stx-vars-jay.rkt"]
◊ul{
