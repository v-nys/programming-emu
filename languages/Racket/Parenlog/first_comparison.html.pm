...
We have now produced enough code to draw a meaningful comparison between our own code and Jay's. To help you get a foothold, here's a table containing bits of code we've already written and their counterparts in Jay's code (which I'm not assuming you've read, but I will be providing some samples from Jay's implementation).
...
Jay's version of ◊code{compile-rule}, however, is quite different from what we wrote. Specifically, it's written as a macro, rather than as a plain old function. This allows extracing Prolog variable identifiers and defining those variables, rather than using ◊code{apply-variable-substitution}. ◊aside{If you need an introduction to macros, ◊a[#:href "http://www.greghendershott.com/fear-of-macros/"]{Greg Hendershott's article} is mandatory reading for the aspiring Racketeer.}
Here is a comparison between a slightly simplified version of Jay's implementation and our existing implementation. You don't have to understand everything that's going on in Jay's version, but do your best to see how the highlighted parts fit into the bigger picture.
...
At this point you might be wondering if the syntax-based approach is worth it. My own impression is that, for now, it doesn't seem necessary. Having ◊code{compile-rule} as syntax prevents us from mapping it, for instance, as is apparent from the tests. The second approach does avoid tracking duplicate symbols and replacing all of those with the same renamed variable, but that isn't particularly hard in this case. The surface syntax is a bit leaner because we don't wrap the arguments to ◊code{compile-rule} rules inside a list, but that seems pretty minor. Regardless, the point is to pick up new ideas, so we'll add the same extensions to both approaches. It is also a pretty common metaprogramming trick to use host language variables to get the desired behavior from guest language variables, so in that sense this approach is quite educational.

We are now ready to rewrite ◊code{compile-rule}. Let's reiterate: because our code still relies on Prolog variables being represented as symbols starting with a capital letter, unbound identifiers have to be bound to such symbols. What's more, every identifier which is unique ◊em{inside that rule} has to be bound to a symbol which cannot occur in any conjunction to which the rule is applied. An easy way to guarantee that is to generate an uninterned symbol, i.e. a symbol which has not yet been created anywhere in the program. The function ◊code{◊a[#:href "http://docs.racket-lang.org/reference/symbols.html#(def._((quote._~23~25kernel)._gensym))"]{gensym}} does just that and it takes an optional prefix, which helps with readability. Here's what I get from the macro expander as the compiled version of one the rules we've used before:
...
◊exercise{Now write the ◊code{compile-rule} macro. Use the expanded rule and tests above as a guideline. If you have to look at the original, that's fine, but don't just memorize. Reprocess the notes, understand how the original works and then start from scratch.}
