...
Parenlog is a logic programming language. Logic programming is its own programming paradigm, like object-oriented or functional programming. There was a time when people thought it would become just as widespread as those other paradigms, but the fact is that it did not. Therefore, I won't assume that you have any experience with it. Don't be fooled into thinking that logic programming is not relevant, however. There are ◊a[#:href "https://www.cs.nmsu.edu/ALP/2010/06/25-years-of-applications-of-logic-programming-in-italy/"]{lots of industrial codebases} and fields that rely on logic programming. It also tends to be popular ◊a[#:href "https://arxiv.org/abs/1802.07284"]{in specific fields}. And it's pretty fun, to boot. Logic programming, in my experience, often feels like doing a puzzle. In short, logic programming is worth having in your toolkit and a logic programming language makes for a fun project.
...
You'll agree that Ares and Athena are friends. So will Prolog. Now, you could pretty easily encode computations that tell you this into more or less any language, but in Prolog there's really nothing to encode other than the knowledge. We'll go into detail a bit further on, but code which lets you use the above information just looks like this:
...
◊ol['((start "4"))]{
  ...
  ◊li{Prime numbers are a part of mathematics.}
  ◊li{Triangles are a part of mathematics.}
  ◊li{Logic is a part of mathematics.}
  ◊li{Eratosthenes likes prime numbers.}
  ◊li{Pythagoras likes triangles.}
  ◊li{Athena likes logic.}
  ◊li{Anyone who likes a part of a whole also likes the whole.}
...
Even if you don't know the exact syntax yet, this looks like a very concise and readable representation of our theory, right? Let's go into a bit more detail regarding the structure of this theory before we start doing computations. First, ◊predicate{friends(X,Y)}, ◊predicate{likes(eratosthenes,primes)},... are all ◊glossaryterm[#:canonical "predicate" #:explanation "in logic programming, a term indicating that its argument belongs to a certain set of values (in the case of one argument) or a relation between its arguments (in the case of multiple arguments), or (second meaning) a mathematical relation between sets to which those arguments belong"]{predicates}. These are ◊glossaryterm[#:canonical "term" #:explanation "Quite generally: syntactic components. In logic programming: logic variables, atoms, or compounds"]{terms} which express something which holds, i.e. which is "true" in the system under consideration. Prolog's terms consist of ◊glossaryterm[#:canonical "logic variable" #:explanation "a mathematical unknown which can be bound only once to a term"]{logic variables} (which begin with an upper case letter and are best considered as "mathematical unknowns" rather than being like variables in most other languages), ◊glossaryterm[#:canonical "atom" #:explanation "in logic programming (assuming no extra-logical features such as built-in arithmetic), a constant"]{atoms} (which begin with a lower case letter and which I would compare to symbols in Racket), or ◊glossaryterm[#:canonical "compound" #:explanation "a term consisting of a symbol and several nested terms"]{compounds} (which begin with a lower case letter and have other terms as their arguments, so predicates are represented as either atoms or as compounds that only occur in the outermost syntactic position).

Encoded information can take the form of a ◊glossaryterm[#:explanation "in logic programming, an axiomatic encoding of information"]{fact}, which is just "raw", axiomatic information or that of a ◊glossaryterm[#:explanation "in logic programming, an encoding of an allowed deduction"]{rule}, which is knowledge that can be used in a deduction. Facts are written as predicates immediately followed by a period (e.g. ◊code{likes(ares,battle).}) and rules are written as one predicate, followed by the ◊glossaryterm[#:explanation "in logic programming, an operator expressing \"if A, then B\""]{implication} symbol ◊code{:-}, followed by a non-zero number of predicates and terminated by a period. Facts and rules are collectively known as ◊glossaryterm[#:canonical "clause" #:explanation "either a fact or a rule"]{clauses}.
