...
What gives? A scoping issue, that's what. Our query is ◊code{likes(athena,X)} and the head of the third applicable rule is ◊code{likes(X,Y)}, which leads to the unification of ◊code{likes(athena,X)} and ◊code{likes(X,Y)}. Those two predicates unify, but the result is ◊code{{X=athena, Y=athena}}. In other words, variables are aliased when they shouldn't be. But there's an easy fix: if none of the variables in any rule we apply also occur in the query to which the rule is applied, there will never be any incorrect aliasing. Therefore, if we rename all variables in a rule (represented as an S-expression list), so that they are ◊glossaryterm[#:explanation "in logic programming, indicates that a variable does not currently occur in any existing construct"]{fresh}, there will be nothing to worry about. In other words, this is Prolog's way of implementing lexical scope.
◊exercise{Write code which renames all the variables in a rule.}
Here is a quick test:
◊listing[#:source "code/rename-variables-test.rkt"]
And here's how I did it:
◊listing[#:source "code/rename-variables.rkt"]
...
◊listing[#:source "code/generator-example.rkt"]
...
◊listing[#:source "code/naive-resolution-example.rkt"]
...
◊listing[#:source "code/resolution.rkt"]
...
◊listing[#:source "code/dereferenced-resolution-example.rkt"]

Here's my version:
◊listing[#:source "code/restrict.rkt"]