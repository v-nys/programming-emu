...
What gives? A scoping issue, that's what. Our query is ◊code{likes(athena,X)} and the head of the third applicable rule is ◊code{likes(X,Y)}, which leads to the unification of ◊code{likes(athena,X)} and ◊code{likes(X,Y)}. Those two predicates unify, but the result is ◊code{{X=athena, Y=athena}}. In other words, variables are incorrectly aliased. But there's an easy fix: if none of the variables in any rule we apply also occur in the query to which the rule is applied, there will never be any incorrect aliasing. Therefore, if we rename all variables in a rule (represented as an S-expression list), so that they are ◊glossaryterm[#:explanation "in logic programming, indicates that a variable does not currently occur in any existing construct"]{fresh}, there will be nothing to worry about.
